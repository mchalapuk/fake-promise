...
class FakePromise {
    ...
    get promiseTrace() { return this._promiseTrace; }
    get resultTrace() { return this._resultTrace; }
    get errorTrace() { return this._errorTrace; }
    get specifyTrace() { return this._specifyTrace; }
    get resolveTrace() { return this._resolveTrace; }
    get rejectTrace() { return this._rejectTrace; }
    ...
    setResult(result) {
        this.check(!this.errorSet, 'trying to set result on a promise with error already set', this._errorTrace);
        this.check(!this.resultSet, 'result already set', this._resultTrace);
        this.check(!this.resultPromised, 'result already set (waiting for promise)', this._promiseTrace);
        if (isPromise(result)) {
    ...
    setError(error) {
        this.check(!this.resultSet, 'trying to set error on a promise with result already set', this._resultTrace);
        this.check(!this.errorSet, 'error already set', this._errorTrace);
        this.check(!this.resultPromised, 'result already set (waiting for promise)', this._promiseTrace);
    ...
    resolveOne(result) {
        this.check(!this.errorSet, 'trying to resolve a promise containing error', this._errorTrace);
        if (result !== undefined) {
            this.setResult(result);
        }
        this.markResolved();
    ...
    rejectOne(error) {
        this.check(!this.resultSet, 'trying to reject a promise containing result', this._resultTrace);
        if (error !== undefined) {
            this.setError(error);
        }
